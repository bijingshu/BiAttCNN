...
class DoubleLSTMClassifier(nn.Module):
    def __init__(self, embedding_dim, lstm_dim, dropout, device):
        ...
        self.lstm = nn.LSTM(embedding_dim, self.hidden_size, num_layers=2,
                            bidirectional=True, batch_first=True, dropout=dropout)
        # CNN
        self.CNN = nn.Sequential(nn.Conv1d(60, 64, 1),
                                 nn.BatchNorm1d(64),
                                 nn.ReLU(),
        ...
        # MLP
        self.hidden_layer = nn.Linear(32, lstm_dim)
    ...
    def forward(self, tcrs, tcr_lens, peps, pep_lens):
        ...
        # BiLSTM Acceptor
        H, _ = self.lstm(tcr_embeds)
        # Attention Network
        M = self.tanh1(H)#[50,25,60]
        alpha = F.softmax(torch.matmul(M, self.w), dim=1).unsqueeze(-1)
        tcr_last_cell = H * alpha 
        # peptide Encoder:
        # Embedding
        pep_embeds = self.pep_embedding(peps)
        # BiLSTM Acceptor
        H, _ = self.lstm(pep_embeds)
        # Attention Network
        M = self.tanh1(H)
        alpha = F.softmax(torch.matmul(M, self.w), dim=1).unsqueeze(-1)
        pep_last_cell = H * alpha 
        # CNN
        tcr_pep_concat = torch.cat([tcr_last_cell, pep_last_cell],1) 
        tcr_pep_concat = tcr_pep_concat.permute(0, 2, 1) 
        tcr_pep_concat = self.CNN(tcr_pep_concat)
        tcr_pep_concat = tcr_pep_concat.permute(0, 2, 1)
        tcr_pep_concat = torch.sum(tcr_pep_concat,1)
        # MLP Classifier
